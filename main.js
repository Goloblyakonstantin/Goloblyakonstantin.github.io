...
class Thing
{
  ...
  updateSelf()  //убрат повороты дир для КСС
  {
...
const boy = new Thing(cells[51][51],{
  type: "boy",
  dir: "S",
  degree: 0,  //захотелось назвать градусом
  state: "walking",
  name: "Я",
  necrolog: "Я умер.",
  body: "deadBoy",
...
boy.turn = side =>
{
  ...
  switch (boy.dir+"->"+side) //свитч именно для трансформа а не дирекшена движения(зависит от дирекшена, конечно)
  {
    case "N->right": case "S->right": case "W->right": case "E->right": boy.rotate("degree","90");  break; //право(написал сразу 90 и -90 чтобы потом меньше умножений было на коэффициенты в функции)
    case "N->left": case "S->left": case "W->left": case "E->left": boy.rotate("degree","-90"); break; //лево
  }
}
